...
const CreationCanvas: React.FC<CreationCanvasProps> = ({ onGenerate, isGenerating }) => {
  const [prompt, setPrompt] = useState('');
  const [style, setStyle] = useState<ArtStyle>(ArtStyle.PIXAR_3D);
  const [tone, setTone] = useState<BookTone>(BookTone.INSPIRATIONAL);
  const [audience, setAudience] = useState('Children (5-8)');
    ...
    <div className="w-full flex flex-col items-center pb-32 animate-fadeIn">
      ...
      <div className="text-center space-y-4 mb-12 mt-8 md:mt-16">
        <h1 className="font-heading font-bold text-5xl md:text-6xl text-charcoal-soft mb-4">
          What will you <span className="text-transparent bg-clip-text bg-gradient-to-r from-coral-burst to-gold-sunshine">create today?</span>
        </h1>
        <p className="font-body text-xl text-cocoa-light max-w-2xl mx-auto">
          Your imagination, beautifully bound. Describe your idea, and let Genesis handle the rest.
        </p>
      </div>
      ...
      <div className="w-full max-w-4xl px-4">
        <div className="bg-white rounded-[32px] shadow-soft-lg p-8 md:p-12 border border-white/50 relative">
                ...
                <div className="space-y-3">
                    ...
                    <div className="relative">
                        <input 
                            ...
                            className="w-full bg-white border-2 border-peach-soft rounded-2xl p-4 font-body text-charcoal-soft focus:outline-none focus:border-coral-burst transition-colors"
                        />
